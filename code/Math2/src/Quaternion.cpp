...
FORCE_INLINE const Quaternion operator | (const Quaternion& a, const Vec<Real, 3>& b)
{
	...
	c[1] = a[0]*b[0] - b[1]*a[3] + a[2]*b[2];
	c[2] = a[0]*b[1] + b[0]*a[3] - a[1]*b[2];
	c[3] = -b[0]*a[2] + a[1]*b[1] + a[0]*b[2];
	return c;
}
...
FORCE_INLINE const Quaternion operator * (const Quaternion& a, const Quaternion& b) 
{
	...
	c[1] = b[0]*a[1] + a[0]*b[1] - b[2]*a[3] + a[2]*b[3];
	c[2] = b[0]*a[2] + a[0]*b[2] + b[1]*a[3] - a[1]*b[3];
	c[3] = -b[1]*a[2] + a[1]*b[2] + b[0]*a[3] + a[0]*b[3];
	return c;
}
...
FORCE_INLINE const Quaternion operator | (const Vec<Real, 3>& a, const Quaternion& b) 
{
	...
	c[1] = b[0]*a[0] - b[2]*a[2] + a[1]*b[3];
	c[2] = b[0]*a[1] + b[1]*a[2] - a[0]*b[3];
	c[3] = -b[1]*a[1] + a[0]*b[2] + b[0]*a[2];
	return c;
}
